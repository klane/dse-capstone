...
def train_model(model, X, Y, learning_rate, output_steps, batch_size, train_idx, valid_idx, test_idx, test=False, return_pred=False):
    ...
    model_path = '/models-vol2/trained_model.pth'
    results_path = '/models-vol2/results_dict.pkl'
    ...
    if exists(model_path):
        ...
        print(datetime.now(), ' Model Loaded...', flush=True)
    ...
    if exists(results_path):
        ...
        fout.close()
        print(datetime.now(), ' Results Loaded...', flush=True)
        write_log('Results Loaded...')
    else:
    ...
    for i in range(200):
        ...
        print(datetime.now(), ' Model Saved...', flush=True)
        ...
        print(datetime.now(), ' Results Saved...', flush=True)
...
model, loss, preds, min_valid_loss, test_rmse = train_model(
    model, X, Y, learning_rate, output_steps = output_steps, batch_size = 512,
